...
const PendingOrderView = ({ order, refetch }) => {
  const {
    TxID,
    card_issuer,
    carts,
  ...
  // Function to handle status change (for dropdown)
  const handleStatusChange = (e) => {
    const newStatus = e.target.value;
    confirmationAlert({confirmButtonText:'Yes! Update'}).then(async(result)=>{
      if(result.isConfirmed){
        try{
          setLoading(true)
          const res = await axiosSecure.patch(`/orders-update/${_id}`, {new_order_status : newStatus, orderStatusChangedBy : user.email}); 
  ...
  return (
    <div className="bg-gray-100 border border-yellow-500 dark:bg-gray-800 shadow-md rounded-lg p-4 mb-4 md:p-6">
      {/* Order Header */}
      ...
      <div className="flex justify-between md:items-start items-center mb-4">
        ...
        <div className="text-right">
          ...
          <p className="md:text-2xl text-lg text-[#ff3811]">
            Due: {due} BDT
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        >
          <option className="text-yellow-500  text-xl" value="Pending">
            Pending
          </option>
          <option className="text-green-500 text-xl" value="Confirm">
            Confirm
          </option>
